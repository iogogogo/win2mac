...
| 文件 | 引擎 | 说明 |
|---|---|---|
| `win2mac.ahk` | AutoHotkey **v1.x** | 主版本，日常使用推荐。功能见下文第 1–4 节。 |
| `win2mac_v2.ahk` | AutoHotkey **v2.x** | 与 v1 功能对齐的 AHK 2.x 兼容版，并额外支持方向键相关的行首/行尾跳转与选中（第 5–6 节）。 |

> 两个版本语法不互通，但可同时安装引擎、各自单独运行。新项目建议用 v2 引擎跑 `win2mac_v2.ahk`；v1 脚本跑得好好的则无需迁移。
...
### 6. Alt + Shift + ← / → 选中到行首 / 行尾（v2 版本专属）

对应 macOS 的 `⌘+Shift+←` / `⌘+Shift+→`。在 `Alt+方向键` 基础上叠加 `Shift`，映射为 `Shift+Home` / `Shift+End`：

| 组合键 | 实际发送 | 作用 |
|---|---|---|
| `Alt+Shift+←` | `Shift+Home` | 从光标选中到行首 |
| `Alt+Shift+→` | `Shift+End` | 从光标选中到行尾 |

> 同样沿用第 5 节的排除规则，浏览器与资源管理器走原生行为。
> 补充：macOS 里 `⌥+←/→`（按单词跳）在 Windows 本就对应 `Ctrl+←/→`，无需映射，直接按 `Ctrl+方向键` 即可。
